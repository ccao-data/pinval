...
if (cache_files %>% sapply(file.exists) %>% all()) {
  ...
  # Set prior year for characteristic queries, since chars are
  ...
  # Load chars that are stored with the PIN-level assessment data
  assessment_pin_df <- open_dataset(
    ...
    select(loc_property_address, pred_pin_final_fmv) %>%
    collect() %>%
  ...
  subject_property_address <- assessment_pin_df %>% pull(property_address)
  pred_pin_final_fmv <- assessment_pin_df %>% pull(pred_pin_final_fmv)
  ...
  assessment_df <- open_dataset(
    ...
    mutate(
      ...
      pred_pin_final_fmv = pred_pin_final_fmv
    )
...
# @param is_multicard Boolean indicating whether the subject parcel is multicard
# @return Returns a list of htmltools elements that can be rendered using `tagList()`
html_tags_for_report <- function(assessment_df, comp_df, is_multicard) {
  return(
    withTags(
      list(
        h3("Location"),
        ...
        h3("Predicted value"),
        pred_fmv_summary(assessment_df, is_multicard)
      )
    )
  )
}
...
# the subject property
pred_fmv_summary <- function(df, is_multicard) {
  # Use the card-specific value in the case of multicard parcels
  pred_fmv <- ifelse(
    is_multicard,
    df$pred_card_final_fmv,
    df$pred_pin_final_fmv
  )
  ...
  return(
    HTML(
      assessment_df %>%
        ...
        bind_rows(
          comp_df %>%
            # Cast comp_num to character so it can share the same type when
            # being joined to the subject property data, which uses an empty
            # string to indicate a null value
...
if (nrow(assessment_df) > 1) {
  ...
  html_tags <- html_tags_for_report(assessment_df, comp_df, is_multicard = FALSE)
}
