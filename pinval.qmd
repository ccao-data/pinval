...
cache_files <- list(
  ...
  metadata = glue::glue("{base_cache_file_path}-meta.RDS")
)
...
if (cache_files %>% sapply(file.exists) %>% all()) {
  ...
  metadata_list <- readRDS(cache_files$metadata)
} else {
  ...
  all_predictors <- model_metadata_df %>%
  ...
  saveRDS(metadata_list, cache_files$metadata)
}
```

This is an experimental home value report. The goal of this report is to explain how the Cook County Assessor's statistical model predicted a subject home’s value. The `r format(metadata_list$final_model_run_date, "%Y")` model was run on **`r format(metadata_list$final_model_run_date, "%B %d, %Y")`**, and it used characteristics and sales available at that time to learn about the real estate market and estimate what the subject home would sell for if it sold on **January 1, `r format(metadata_list$final_model_run_date, "%Y")`**. The sales that the model used for estimation are reported by the Illinois Department of Revenue, which records sales using the [Real Estate Transfer Declaration form](https://tax.illinois.gov/content/dam/soi/en/web/tax/localgovernments/property/documents/ptax-203.pdf).
...
## `r metadata_list$subject_property_address`
  ...
  return(
    HTML(
      paste0(
        ...
        format(metadata_list$final_model_run_date, "%B %d, %Y"),
        " predicted that the value of this home as of lien date January 1, ",
        format(metadata_list$final_model_run_date, "%Y"),
        " should be ",
  ...
  return(
    assessment_df %>%
      ...
      select(
        ...
        "Assessor Neighborhood" = meta_nbhd_code,
        all_of(top_chars),
        all_of(metadata_list$all_predictors[!metadata_list$all_predictors %in% top_chars])
      ) %>%
      bind_rows(
        comp_df %>%
          select(
      ...
      mutate(across(starts_with("Percent"), scales::label_percent(accuracy = 1))) %>%
      ...
      freeze_first_n_rows(n = 2, rows_per_page = 10) %>%
      ...
      reactable(
        ...
        sortable = FALSE,
        columns = list(
          Characteristic = colDef(style = list(fontWeight = "bold"))
        ),
        rowStyle = function(index) {
          styles <- list()
          if (.[index, "Characteristic"] %in% c("Sale Price", "Sale $/sqft")) {
            styles <- c(styles, background = "rgba(0, 0, 0, 0.05)")
          }
          if (.[index, "Characteristic"] == "Sale $/sqft") {
