...
# Load PIN-level assessment data for chars that are missing from card data
assessment_pin_df <- open_dataset(
  ...
  select(meta_pin, loc_property_address) %>%
  collect() %>%
...
## `r assessment_pin_df$property_address`

### "Property ID number: `r sprintf('<a target=_blank href=https://www.cookcountyassessor.com/pin/%s>%s</a>', params$pin, params$pin)`"

This is an experimental AutoAppraiser report. The goal of this report is to explain how the statistical model predicted a subject home’s value. The `r substr(params$run_id, 1, 4)` model was run on `r substr(params$run_id, 1, 10)`, and it used characteristics and sales available at that time to learn about the real estate market and estimate what the subject home would sell for if it sold on January 1, `r substr(params$run_id, 1, 4)`.
...
comp_factors <- c(
  # Include subject property in the factor vector so that we can use it
  # in the map legend
  "Subject property",
  "Comp 1 (most similar)",
  paste0("Comp ", 2:4),
...
leaflet_assessment_df <- assessment_df %>%
  # Anything in the assessment df is a subject property
  mutate(comp_num_factor = comp_factors[1])

leaflet_comp_df <- comp_df %>%
  # Skip the first index of `comp_factors`, since it's used for the target PIN
  mutate(comp_num_factor = comp_factors[comp_num + 1])

color_palette <- colorFactor(
  c("black", "#004d00", "#006600", "#008000", "#00cc00", "#99ff99"),
  comp_factors,
  ordered = TRUE
)

# Function to get the label text for comps, which should show sale price and
# sale date
comp_label <- function(sale_price, sale_date) {
  formatted_sale_price <- scales::dollar(sale_price)
  formatted_sale_date <- sale_date %>% as.Date %>% format("%h %Y")
  return(glue::glue("{formatted_sale_price} ({formatted_sale_date})"))
}

leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  # TODO: Label this dot as the target
  addCircleMarkers(
    data = leaflet_assessment_df,
    ~loc_longitude,
    ~loc_latitude,
    opacity = 1,
    fillOpacity = 1,
    radius = 2,
    color = ~ color_palette(comp_num_factor),
    label = comp_factors[1],
  ...
  addCircleMarkers(
    ...
    radius = 2,
    color = ~ color_palette(comp_num_factor),
    label = ~ comp_label(meta_sale_price, meta_sale_date),
    labelOptions = labelOptions(
      noHide = FALSE,
      textsize = "12px",
      direction = "top",
    ...
    popup = ~ paste0(
      ...
      "<hr>",
      "<b>Sale price</b>: ", scales::dollar(meta_sale_price, accuracy = 1),
      "<br><b>Sale date</b>: ", meta_sale_date,
...
_TK: Table comparing target PIN chars to comp chars_
