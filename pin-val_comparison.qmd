...
# 1) Build the base path for caching
base_cache_file_path <- glue::glue(
  "cache/{params$run_id}_{params$pin1}_{params$pin_1_card_num}_{params$pin2}_{params$pin_2_card_num}"
)

# 2) List all files you want to cache
cache_files <- list(
  ...
  comps      = glue::glue("{base_cache_file_path}_comps.parquet")
)
...
# 3) If ALL files exist, read them from disk; else run the code and write them out
if (all(map_lgl(cache_files, file.exists))) {
  message("All cache files found. Reading from disk...")
  
  # -- READ all cached data frames --
  ...
  char_df       <- read_parquet(cache_files$char)
  value_df      <- read_parquet(cache_files$value)
  shap_df       <- read_parquet(cache_files$shap)
  address_df    <- read_parquet(cache_files$address)
  sales_df      <- read_parquet(cache_files$sales)
  comps_df      <- read_parquet(cache_files$comps)

} else {
  message("Some or all cache files are missing. Running queries and creating them...")

  #
  # ================= YOUR EXISTING “MIDDLE” CODE =================
  #
  # Everything below is exactly your existing code:
  # ---------------------------------------------------------------
  
  # e.g. if (cache_files %>% sapply(file.exists) %>% all()) { ...
  #   # That old `if` can be removed or replaced, because we are now
  #   # wrapping the entire code inside this else block.
  # }

  # (1) We'll assume your queries to get 'assessment_df', 'comp_df', 'metadata_df' happen here:
  # For example:
  #   assessment_df <- read_parquet(...)
  #   comp_df       <- read_parquet(...)
  #   ...
  ...
  pin_labels <- pin_labels %>%
    left_join(
      char_df %>% select(meta_pin, primary_card) %>% distinct(),
      by = "meta_pin",
      suffix = c("", ".computed")
    ) %>%
    mutate(
  ...
  write_parquet(char_df,       cache_files$char)
  write_parquet(value_df,      cache_files$value)
  write_parquet(shap_df,       cache_files$shap)
  write_parquet(address_df,    cache_files$address)
  write_parquet(sales_df,      cache_files$sales)
  write_parquet(comps_df,      cache_files$comps)
}
...
sales <- sales %>%
  ...
  # Select and order columns for display; note the inclusion of comp info
  select(pin, sale_date, sale_price, doc_no, comp_score, comp_number, other_pin_is_comp) %>%
