...
# Now create a new column that returns the comp_pin column name where pin matches
comps <- comps %>%
  mutate(
    matching_comp = map2_chr(pin, comp_pins, ~ {
      # .x is the current row's pin value, .y is the named vector from comp_pins
      match_idx <- which(.y == .x)
      if (length(match_idx) > 0) {
        # If more than one match, return a comma-separated string of the column names.
        paste(names(.y)[match_idx], collapse = ", ")
      } else {
        NA_character_
      }
    })
