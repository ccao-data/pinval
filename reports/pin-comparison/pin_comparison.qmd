...
Comparison between PINs <span style="color: red;"><b>`r params$pin1`</b></span> and <span style="color: blue;"><b>`r params$pin2`</b></span>
...
datatable(
  ...
  options = list(
    ...
    headerCallback = JS(sprintf(
      "function(thead, data, start, end, display) {
         $('th:contains(\"%s\")', thead).css({'color': 'red', 'font-weight': 'bold'});
         $('th:contains(\"%s\")', thead).css({'color': 'blue', 'font-weight': 'bold'});
       }",
      params$pin1, params$pin2
    ))
  )
...
```
### Percent of Total Absolute SHAP Values
```{r}
#| column: screen-inset

shap %>%
  mutate(across(2:3, ~ (. / sum(abs(.))) * 100)) %>%
  ggplot(aes(
      y = `Feature Name`,
      x = .data[[params$pin1]],
      xend = .data[[params$pin2]]
