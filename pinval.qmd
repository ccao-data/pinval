...
base_cache_file_path <- glue::glue("cache/{params$run_id}_{params$pin}")
cache_files <- list(
...
if (cache_files %>% sapply(file.exists) %>% all()) {
  ...
  comp_df <- comp_df %>%
    ...
    ccao::vars_recode(
      ...
      sale_date = as.Date(meta_sale_date) %>%
        format("%b %Y")
    )
  ...
  leaflet() %>%
    ...
    addCircleMarkers(
      ...
      popup = ~ paste0(
        ...
        "<br><b>Property class</b>: ", char_class,
        "<br><b>Assessor neighborhood</b>: ", meta_nbhd_code,
        "<br><b>Sale price</b>: ", meta_sale_price,
