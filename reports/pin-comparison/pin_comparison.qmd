...
if (all(map_lgl(cache_files, file.exists))) {
  ...
  if (year <= 2024 &&
    ...
    stop("Error: For PINs with <= 3 cards and 2024 or before,
         the `pin_{x}_card_num` param must be present.")
  }
...
`r if (any(value$total_cards %in% c(2, 3), na.rm = TRUE) && any(value$year >= 2025, na.rm = TRUE)) "If a PIN has 2 or 3 cards, we select features from the card with the largest building square footage. Ties are broken by the lower PIN number. Building square footage is summed in accordance with our modeling technique."`
