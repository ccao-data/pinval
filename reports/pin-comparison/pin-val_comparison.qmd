...
base_cache_file_path <- glue::glue(
  "reports/pin_comparison/cache/{coalesce_empty(params$run_id)}_",
...
dir.create(dirname(base_cache_file_path), recursive = TRUE, showWarnings = FALSE)


cache_files <- list(
  char       = glue::glue("{base_cache_file_path}/char.parquet"),
  value      = glue::glue("{base_cache_file_path}/value.parquet"),
  shap       = glue::glue("{base_cache_file_path}/shap.parquet"),
  address    = glue::glue("{base_cache_file_path}/address.parquet"),
  sales      = glue::glue("{base_cache_file_path}/sales.parquet")
)
...
if (all(map_lgl(cache_files, file.exists))) {
  ...
  # Validation checks - we want card to be empty unless the PIN has more than 4 cards
  ...
  # Add primary card for single card PINs
  pin_labels <- pin_labels %>%
  ...
  comps <- comps %>%
    ...
    select(-primary_card) %>%
    # Create a row_id to know the order of rows
    mutate(row_id = row_number()) %>%
    # For exactly two rows: if row 1 then inverse_pin is the next row's pin; if row 2 then inverse_pin is the previous row's pin
    mutate(inverse_pin = if_else(row_id == 1, lead(pin, 1), lag(pin, 1)))
  ...
  comps <- comps %>%
    rowwise() %>%
    mutate(
      # Use inverse_pin to find the matching index among comp_pin columns
      matching_index = {
        idx <- which(c_across(starts_with("comp_pin")) == inverse_pin)
  ...
  # Join with sales using both pin and doc_no matching comp_doc_no
  sales <- sales %>%
    left_join(comps, by = c("pin" = "inverse_pin", "doc_no" = "comp_doc_no")) %>%
    mutate(
      sale_date = substr(sale_date, 1, 10)
    ) %>%
    select(pin, sale_date, sale_price, sv_is_outlier, doc_no, comp_score, comp_number) %>%
...
Note that for PINs with 2 or 3 cards, the building square footage is summed. For these PINs, they are pulled from the larger of the PINs. This coincides with our modeling techniques. For PINs with 4 or more pins, we pull the features from the target card identified in the params.
...
sales <- sales %>%
  mutate(
    other_pin_is_comp = ifelse(!is.na(comp_number), "✅", ""),
...
pin_colors <- sapply(unique_pins, function(p) {
  # It’s assumed that every pin has a unique color.
