...
## `r assessment_pin_df$property_address` (Property ID `r link_pin(params$pin)`)

This is an experimental AutoAppraiser report. The goal of this report is to explain how the statistical model predicted a subject home’s value. The `r substr(params$run_id, 1, 4)` model was run on `r params$run_id %>% substr(1, 10) %>% as.Date() %>% format("%B %d %Y")`, and it used characteristics and sales available at that time to learn about the real estate market and estimate what the subject home would sell for if it sold on January 1 `r substr(params$run_id, 1, 4)`.
...
**`r avg_sale_price`** at **`r avg_price_per_sqft`/sq. ft.**.

```{r}
assessment_df %>%
  mutate(
    property_address = assessment_pin_df$property_address,
    comp_num = "",
