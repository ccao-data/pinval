...
if (all(map_lgl(cache_files, file.exists))) {
  ...
  if (nrow(pin_labels %>% filter(total_cards <= 3 & !is.na(primary_card))) > 0) {
    stop("Error: For PINs with <= 3 cards, the `pin_{x}_card_num` param must be empty.")
  }
  if (nrow(pin_labels %>% filter(total_cards >= 4 & is.na(primary_card))) > 0) {
    stop("Error: For PINs with >= 4 cards, the `pin_{x}_card_num` param must have a value.")
  }
  ...
  # Identify the appended comp_pin columns (e.g. "comp_pin_1", "comp_pin_2", etc.)
  comp_pin_cols <- grep("^comp_pin_", names(comps), value = TRUE)

  # For each appended comp_pin column, create new columns for the document number and comp score
  # if the comp_pin value matches a primary pin.
  for (col in comp_pin_cols) {
    # Extract the comp_number number from the column name (e.g. "1" from "comp_pin_1")
    ...
    # Create new columns with a prefix "matched_"
    comps <- comps %>%
      mutate(
        # If the comp_pin value is not NA, is different from the current row's primary pin,
        # and appears in the list of primary pins, then copy the corresponding doc_no value;
        # otherwise, set to NA.
        !!paste0("matched_doc_no_", comp_number) := if_else(
          !is.na(.data[[col]]) & (.data[[col]] != pin) & (.data[[col]] %in% pin_labels$meta_pin),
  ...
  # Pivot the matched columns together using a names_pattern
  comps <- comps %>%
    pivot_longer(
  ...
  # Now merge with sales based on doc_no.
  # This join will add the comp_number and its comp_score to the sale row.
  sales <- sales %>%
    left_join(comps, by = "doc_no") %>%
    arrange(desc(sale_date)) %>%
    select(pin, year, sale_date, doc_no, sale_price, sv_is_outlier,
...
Comparison between PINs <span style="color: red;">`r params$pin1`</span> and <span style="color: blue;">`r params$pin2`</span>
...
if (any(value$total_cards > 1, na.rm = TRUE)) {
  cat("If either PIN is a multi-card, values are produced with all cards summed as well as those of the target card.\n")
}
  ...
  options = list(
    ...
    headerCallback = JS(sprintf(
      "function(thead, data, start, end, display) {
         $('th:contains(\"%s\")', thead).css('color', 'red');
         $('th:contains(\"%s\")', thead).css('color', 'blue');
       }",
...
`r if (any(value$total_cards %in% c(2, 3), na.rm = TRUE)) "If a PIN has 2 or 3 cards, we select features from the card with the largest building square footage. Ties are broken by the lower PIN number. Building square footage is summed in accordance with our modeling technique."`

```{r feature_comparison, out.width="90%"}
if (any(value$total_cards > 3, na.rm = TRUE)) {
  cat("If a PIN has 4 or more cards, we use features from the target card.\n")
}
...
char_fmt <- char %>%
  ...
  # Convert everything to character so that we can pivot and combine feature value columns
  ...
  # Pivot back to wide so that each property has its own column representing the value for each feature.
  pivot_wider(
    names_from = meta_pin,
    values_from = Value
  ) %>%
...
datatable(
  ...
  options = list(
    ...
    scrollX = TRUE,
    scrollY = "400px",
    # Use a headerCallback to color the header cells only
    headerCallback = JS(sprintf(
      "function(thead, data, start, end, display) {
         $('th:contains(\"%s\")', thead).css('color', 'red');
         $('th:contains(\"%s\")', thead).css('color', 'blue');
...
sales <- sales %>%
  mutate(
    other_pin_is_comp = ifelse(!is.na(other_pin_comp_number), "✅", ""),
    sale_price = dollar(as.numeric(sale_price)),
    other_pin_comp_score = round(other_pin_comp_score, 4),
    sale_date = substr(sale_date, 1, 10),
    color = ifelse(
      pin == params$pin1, "red",
      ifelse(pin == params$pin2, "blue", "green")
    )
  ) %>%
  relocate(other_pin_is_comp, .after = sv_is_outlier)
...
datatable(
  ...
  formatStyle(
    "pin",
    backgroundColor = styleEqual(unique_pins, pin_colors)
  )
```
...
shap %>%
  ...
  labs(
    title = "SHAP differences between the two PINs",
    x = "SHAP Value",
    y = "Feature Name"
  ) +
